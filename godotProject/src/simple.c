...
godot_variant godotMain(godot_object *p_instance, int argc, char const **argv) {
...
out:
  ...
  api->godot_string_new(&data);
  api->godot_string_parse_utf8(&data, "Hallo from Clingo!");
  api->godot_variant_new_string(&gRet, &data);
  api->godot_string_destroy(&data);
